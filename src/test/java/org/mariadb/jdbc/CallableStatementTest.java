...
public class CallableStatementTest extends BaseTest {
    ...
    public void testBug3540() throws Exception {
        createProcedure("testBug3540", "(x int, out y int)\n"
                + "BEGIN\n"
                + "SELECT 1;end\n");
        ResultSet rs = sharedConnection.getMetaData().getProcedureColumns(null, null, "testBug3540%", "%");
        if (rs.next()) {

            assertEquals("testBug3540", rs.getString(3));
    ...
    public void testBug7026() throws Exception {
        ...
        ResultSet rs = sharedConnection.getMetaData().getProcedures(sharedConnection.getCatalog(), null, "testBug7026");
        if (rs.next()) {
            assertTrue("testBug7026".equals(rs.getString(3)));
            assertFalse(rs.next());
        } else {
            fail();
        }
        ...
        //
        rs = sharedConnection.getMetaData().getProcedures("abfgerfg", null, "testBug7026");
        assertFalse(rs.next());

        //
        // Should be found this time as well, as we haven't specified a catalog.
        //
        rs = sharedConnection.getMetaData().getProcedures(null, null, "testBug7026");
        if (rs.next()) {
            assertTrue("testBug7026".equals(rs.getString(3)));
            assertFalse(rs.next());
        } else {
    ...
    public void testBug9319() throws Exception {
        ...
        try {
            db2Connection = setConnection();
            db1Connection = setConnection();
            ...
            try {
                ...
                assertEquals("07004", sqlEx.getSQLState());
            }
            ...
            db1Connection.close();
            db2Connection.close();
            sharedConnection.createStatement().executeUpdate("DROP DATABASE IF EXISTS db_9319_2");
            sharedConnection.createStatement().executeUpdate("DROP DATABASE IF EXISTS db_9319_1");
        }
    }
    ...
    public void testBug9682() throws Exception {
        ...
        try {
            cStmt = sharedConnection.prepareCall("Call testBug9682(?)");
    ...
    public void testBug10310() throws Exception {
        createFunction("testBug10310", "(a float, b bigint, c int) RETURNS INT NO SQL\nBEGIN\nRETURN a;\nEND");
        CallableStatement cStmt = null;
        ResultSet rs = null;
        try {
            cStmt = sharedConnection.prepareCall("{? = CALL testBug10310(?,?,?)}");
            ...
            rs = dbmd.getFunctionColumns(sharedConnection.getCatalog(), null, "testBug10310", "%");
            ...
            rs = dbmd.getProcedures(sharedConnection.getCatalog(), null, "testBug10310");
            rs.next();
            ...
            cStmt = sharedConnection.prepareCall("{? = CALL testBug10310(4,5,?)}");
            cStmt.registerOutParameter(1, Types.INTEGER);
            cStmt.setInt(2, 1);
    ...
    public void testBug12417() throws Exception {
        ...
        try {
            ucCatalogConn = setConnection();
            ucCatalogConn.setCatalog(sharedConnection.getCatalog().toUpperCase());
    ...
    public void testBug15121() throws Exception {
        ...
        sharedConnection.createStatement().executeUpdate("CREATE DATABASE IF NOT EXISTS test2");
        sharedConnection.setCatalog("test2");

        Connection noDbConn = null;

        try {
            noDbConn = setConnection("","test2");
            StringBuilder queryBuf = new StringBuilder("{call ");
            String quotedId = sharedConnection.getMetaData().getIdentifierQuoteString();
            queryBuf.append(quotedId);
            queryBuf.append(sharedConnection.getCatalog());
            queryBuf.append(quotedId);
    ...
    public void testBug15464() throws Exception {
        ...
        storedProc = sharedConnection.prepareCall("{call testInOutParam(?, ?)}");
    ...
    public void testBug17898() throws Exception {
        ...
        CallableStatement cstmt = sharedConnection.prepareCall("{CALL testBug17898('foo', ?)}");
        cstmt.registerOutParameter(1, Types.INTEGER);
        cstmt.execute();
    ...
    public void testBug21462() throws Exception {
        ...
        CallableStatement cstmt = sharedConnection.prepareCall("{CALL testBug21462}");
        cstmt.execute();
    }
    ...
    public void testBug22024() throws Exception {
        createProcedure("testBug22024", "(\r\n)\r\n BEGIN SELECT 1; END");
        CallableStatement callStmt = sharedConnection.prepareCall("{CALL testBug22024()}");
        callStmt.execute();

        sharedConnection.createStatement().executeUpdate("DROP PROCEDURE IF EXISTS testBug22024");
        sharedConnection.createStatement().executeUpdate("CREATE PROCEDURE testBug22024(\r\na INT)\r\n BEGIN SELECT 1; END");
        callStmt = sharedConnection.prepareCall("{CALL testBug22024(?)}");
        callStmt.setInt(1, 1);
        callStmt.execute();
    }
    ...
    public void testBug22297() throws Exception {
        ...
        stmt.executeUpdate("DROP PROCEDURE IF EXISTS testBug22297");
        ...
        stmt.executeUpdate("INSERT INTO tblTestBug2297_1 (`id`,`Income`) VALUES ('a',4094.00),('b',500.00),('c',3462.17), ('d',500.00), ('e',600.00)");

        stmt.executeUpdate("INSERT INTO tblTestBug2297_2 (`id`,`CreatedOn`) VALUES ('d','2006-08-31 00:00:00'),('e','2006-08-31 00:00:00'),"
                + "('b','2006-08-31 00:00:00'),('c','2006-08-31 00:00:00'),('a','2006-08-31 00:00:00')");

        PreparedStatement pstmt = sharedConnection.prepareStatement("{CALL testBug22297(?)}");
        pstmt.setInt(1, 1);
        ...
        assertTrue(pstmt.getClass().getName().indexOf("Server") == -1);
    }
    ...
    public void testHugeNumberOfParameters() throws Exception {
        ...
        try {
            cStmt = sharedConnection.prepareCall("{call testHugeNumberOfParameters(" + param.toString() + ")}");
    ...
    public void testPrepareOfMultiRs() throws Exception {
        ...
        PreparedStatement ps = sharedConnection.prepareStatement("call p()");
        ps.execute();
    ...
    public void testBug25379() throws Exception {
        ...
        CallableStatement cstmt = sharedConnection.prepareCall("{call sp_testBug25379(?)}");
        cstmt.setString(1, "'john'");
        cstmt.executeUpdate();
        assertEquals("'john'", cstmt.getString(1));
        ResultSet rs = sharedConnection.createStatement().executeQuery("SELECT col FROM testBug25379");
        if (rs.next()) {
    ...
    public void testBug25715() throws Exception {
        ...
        try {
            cstmt = sharedConnection.prepareCall("{call spbug25715(?)}");
    ...
    public void testBug26143() throws Exception {
        Statement stmt = sharedConnection.createStatement();
        stmt.executeUpdate("DROP PROCEDURE IF EXISTS testBug26143");
        stmt.executeUpdate("CREATE DEFINER=CURRENT_USER PROCEDURE testBug26143(I INT) COMMENT 'abcdefg'\nBEGIN\nSELECT I * 10;\nEND");
        sharedConnection.prepareCall("{call testBug26143(?)").close();
    }
    ...
    public void testBug26959() throws Exception {
        ...
        sharedConnection.prepareCall("{call testBug26959(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)}").close();
        ResultSet rs = sharedConnection.getMetaData().getProcedureColumns(sharedConnection.getCatalog(), null, "testBug26959", "%");
        ...
        sharedConnection.prepareCall("{call testBug26959_1(?, ?)}").close();

        rs = sharedConnection.getMetaData().getProcedureColumns(sharedConnection.getCatalog(), null, "testBug26959_1", "%");
    ...
    public void testBug27400() throws Exception {
        ...
        try {
            cStmt = sharedConnection.prepareCall("{CALL /* SOME COMMENT */ testBug27400( /* does this work too? */ ?, ?)} # and a commented ? here too");
    ...
    public void testBug28689() throws Exception {
        createTable("testBug28689", "(`id` int(11) NOT NULL auto_increment,`usuario` varchar(255) default NULL,PRIMARY KEY  (`id`))");
        Statement stmt = sharedConnection.createStatement();
        stmt.executeUpdate("INSERT INTO testBug28689 (usuario) VALUES ('AAAAAA')");

        createProcedure("sp_testBug28689", "(tid INT)\nBEGIN\nUPDATE testBug28689 SET usuario = 'BBBBBB' WHERE id = tid;\nEND");

        Connection noProcedureBodiesConn = setConnection("&noAccessToProcedureBodies=true");
        CallableStatement cStmt = null;
        ...
        try {
            ...
            cStmt.addBatch();
            cStmt.executeBatch();
            ResultSet rs = noProcedureBodiesConn.createStatement().executeQuery("SELECT `usuario` FROM testBug28689 WHERE id=1");
            if (rs.next()) {
                assertEquals("BBBBBB", rs.getString(1));
            } else {
                fail();
            }
    ...
    public void testBug31823() throws Exception {
        ...
        try {
            callable = sharedConnection.prepareCall("{? = call f_testBug31823(?,?)}");
    ...
    public void testBug32246() throws Exception {
        ...
        doBug32246(sharedConnection);
        Connection conn = null;
        try {
            conn = setConnection("&useDirectRowUnpack=false");
            doBug32246(conn);
        } finally {
            if (conn != null) {
                conn.close();
    ...
    private void doBug32246(Connection aConn) throws SQLException {
        ...
        aConn.createStatement().executeUpdate("INSERT INTO test_table_1 VALUES (1)");
        createTable("test_table_2", "(value_2 BIGINT PRIMARY KEY) ENGINE=InnoDB");
        aConn.createStatement().executeUpdate("DROP FUNCTION IF EXISTS test_function");
    ...
    public void testBitSp() throws Exception {
        ...
        sharedConnection.createStatement().executeUpdate(Min_Insert);
        cstmt = sharedConnection.prepareCall("{call Bit_Proc(?,?,?)}");
        ...
        ResultSet rs = sharedConnection.createStatement().executeQuery(Min_Val_Query);
    ...
    public void testNotReallyCallableStatement() throws Exception {
        ...
        try {
            sharedConnection.createStatement().executeUpdate("DROP TABLE IF EXISTS testNotReallyCallableStatement");
            cstmt = sharedConnection.prepareCall("CREATE TABLE testNotReallyCallableStatement(field1 INT)");
    ...
    public void testBug35199() throws Exception {
        ...
        CallableStatement callable = null;
        try {
            callable = sharedConnection.prepareCall("{? = call test_function(?,101,?)}");
            callable.registerOutParameter(1, Types.BIGINT);
    ...
    public void testBug49831() throws Exception {
        ...
        execProcBug49831(sharedConnection);
        sharedConnection.createStatement().execute("TRUNCATE TABLE testBug49831");
        Connection noBodiesConn = setConnection("&noAccessToProcedureBodies=true&jdbcCompliantTruncation=false&characterEncoding=utf8&useUnicode=yes");
        try {
    ...
    private void execProcBug49831(Connection c) throws Exception {
        ...
        ResultSet rs = sharedConnection.createStatement().executeQuery("SELECT * from testBug49831 ORDER BY VAL ASC");
        rs.next();
    ...
    public void testBug57022() throws Exception {
        ...
        sharedConnection.createStatement().executeUpdate("CREATE DATABASE IF NOT EXISTS bug57022");

        createProcedure("bug57022.procbug57022", "(x int, out y int)\nbegin\ndeclare z int;\nset z = x+1, y = z;\nend\n");

        CallableStatement cStmt = null;
        try {
            cStmt = sharedConnection.prepareCall("{call `bug57022`.`procbug57022`(?, ?)}");
            cStmt.setInt(1, 5);
            ...
            sharedConnection.setCatalog("bug57022");
            cStmt = sharedConnection.prepareCall("{call bug57022.procbug57022(?, ?)}");
            cStmt.setInt(1, 5);
            cStmt.registerOutParameter(2, Types.INTEGER);
            ...
            sharedConnection.createStatement().executeUpdate("DOP DATABASE bug57022");
        }
    ...
    public void testBug60816() throws Exception {
        ...
        CallableStatement call = sharedConnection.prepareCall("{ call test60816_1(?) }");
        call.setInt(1, 1);
        call.registerOutParameter(1, Types.INTEGER);
        call.execute();
        assertEquals(2, call.getInt(1));

        call = sharedConnection.prepareCall("{ call test60816_2(?, ?) }");
        call.setInt(1, 1);
        ...
        call = sharedConnection.prepareCall("{ call test60816_1(?) }");
        call.setNull(1, Types.INTEGER);
        call.registerOutParameter(1, Types.INTEGER);
        call.execute();
        ...
        call = sharedConnection.prepareCall("{ call test60816_3(?) }");
        call.setNull(1, Types.INTEGER);
        call.registerOutParameter(1, Types.INTEGER);
        call.execute();
